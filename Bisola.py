...
def formula_html(steps):
    html = (
        ...
        '<tr><th>Target Identifier</th>'
        '<th>Calculated Formula Operation Breakdowns</th></tr>'
    )
...
modules = [
    ("EOQ", "Economic Order Quantity", "Classic Wilson EOQ model",    "eoq"),
    ("SS",  "Safety Stock and ROP",    "Buffer stock optimization",   "ss"),
    ("TC",  "Total Inventory Cost",    "System cost distribution",    "tc"),
    ("ALL", "Calculate All",           "Calculate everything",        "all"),
]

st.sidebar.markdown("### Control Engine")
...
if mod == "eoq":
    ...
    with col1:
        ...
        S = st.number_input("Ordering Cost per Order (S, NGN)", min_value=0.0, value=5000.0, step=250.0)
    with col2:
        H = st.number_input("Holding Cost per Unit/Year (H, NGN)", min_value=0.001, value=20.0, step=1.0)
...
# ── 7. MODULE: SAFETY STOCK & ROP ────────────────────────────────────────────
elif mod == "ss":
    st.markdown("### Safety Stock and Reorder Point Engine")
    st.markdown("Calculate strategic buffer levels to hedge against stochastic demand during supplier lead periods.")

    col1, col2 = st.columns(2)
    with col1:
        mu    = st.number_input("Mean Daily Demand (mu)",                    min_value=0.0, value=50.0,  step=1.0)
        sigma = st.number_input("Standard Deviation of Daily Demand (sigma)", min_value=0.0, value=12.5, step=0.5)
    with col2:
        ...
        z    = st.selectbox(
            ...
            format_func=lambda x: {
                1.28: "90% (z=1.28)", 1.65: "95% (z=1.65)",
                1.96: "97.5% (z=1.96)", 2.33: "99% (z=2.33)"
            }[x]
        )
...
# ── 8. MODULE: TOTAL COST ─────────────────────────────────────────────────────
    ...
    if st.button("Analyze System Cost Distribution"):
        ...
        cols_metrics([
            ("Holding Premium",    f"NGN {hc:,.2f}",     "Carrying Cost"),
            ("Ordering Overhead",  f"NGN {oc:,.2f}",     "Procurement Cost"),
            ("Shortage Exposure",  f"NGN {shc:,.2f}",    "Risk Cost"),
            ("Total Combined Cost",f"NGN {tc_val:,.2f}", "Annual Matrix"),
        ], ncols=4)

# ── 9. MODULE: CALCULATE ALL ─────────────────────────────────────────────────
elif mod == "all":
    ...
    with icol1:
        mu_all    = st.number_input("Mean daily demand (mu)",   min_value=0.0, value=50.0,   step=1.0,  key="all_mu")
        sigma_all = st.number_input("Std deviation (sigma)",    min_value=0.0, value=12.5,   step=0.5,  key="all_sigma")
    with icol2:
    ...
    with icol3:
        S_all  = st.number_input("Ordering cost per order (NGN)", min_value=0.0, value=5000.0, step=100.0, key="all_S")
        H_all  = st.number_input("Holding cost per unit/yr (NGN)",min_value=0.0, value=20.0,   step=1.0,   key="all_H")
        SC_all = st.number_input("Shortage cost per unit (NGN)",  min_value=0.0, value=50.0,   step=1.0,   key="all_SC")
    ...
    if st.button("Run Master Calculation", key="calc_all_run"):
        if H_all == 0:
            ...
            # ── Primary Metrics ───────────────────────────────────────────────
            st.markdown('<div class="section-header">Primary Optimization Metrics</div>',
                        unsafe_allow_html=True)
            cols_metrics([
                ...
                ("Total Annual Cost",   f"NGN {tc_all:,.0f}",    "combined system"),
            ], ncols=4)

            # ── Operations Breakdown ──────────────────────────────────────────
            st.markdown('<div class="section-header">Operations Framework Breakdown</div>',
                        unsafe_allow_html=True)
            cols_metrics([
            ...
            # ── Visualizations ────────────────────────────────────────────────
            st.markdown('<div class="section-header">System Optimization Visualizations</div>',
                        unsafe_allow_html=True)
            c1, c2 = st.columns(2)
            ...
            with c1:
                ...
                fig_cost.add_trace(go.Scatter(
                    x=q_range, y=hc_line, name="Holding Cost",
                    line={"color": "#4f46e5", "width": 2}, mode="lines"
                ))
                fig_cost.add_trace(go.Scatter(
                    x=q_range, y=oc_line, name="Ordering Cost",
                    line={"color": "#ec4899", "width": 2}, mode="lines"
                ))
                fig_cost.add_trace(go.Scatter(
                    x=q_range, y=tc_line, name="Total System",
                    line={"color": "#10b981", "width": 3}, mode="lines"
                ))
                fig_cost.add_vline(
                    x=eoq_all, line_dash="dash", line_color="#3b82f6",
                ...
                fig_cost.update_layout(
                    ...
                    font={"family": "DM Sans", "color": "#475569"},
                    yaxis={"title": "Cost (NGN)", "gridcolor": "#e2e8f0", "color": "#475569"},
                    xaxis={"title": "Order Quantity", "color": "#475569"},
            ...
            with c2:
                ...
                fig_wf.update_layout(
                    title={"text": "Reorder Point (ROP) Structure", "font": {"color": "#0f172a", "size": 15}},
                    paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
                    font={"family": "DM Sans", "color": "#475569"},
            ...
            with st.expander("View Consolidated Formula Audit Trail"):
                st.markdown(formula_html([
                    ("Annual Demand (D)",
                     f"{mu_all} x 365 = {D_all:,.0f} units/yr"),
                    ("EOQ = sqrt(2DS/H)",
                     f"sqrt(2 x {D_all:,.0f} x {S_all:,.0f} / {H_all}) = {eoq_all:,.1f} units"),
                    ("Safety Stock (SS)",
                     f"{z_all} x ({sigma_all} x sqrt({lead_all})) = {ss_all:.1f} units"),
                    ("Reorder Point (ROP)",
                     f"{demand_during_lt_all:.1f} + {ss_all:.1f} = {rop_all:.1f} units"),
                    ("Total Cost",
                     f"HC (NGN {hc_all:,.0f}) + OC (NGN {oc_all:,.0f}) + SC (NGN {shc_all:,.0f}) = NGN {tc_all:,.0f}/yr"),
                ]), unsafe_allow_html=True)
    else:
        st.info("Complete all parameter values above and click Run Master Calculation to evaluate your full optimization portfolio.")
modules = [
...
# --- MODULE 1: EOQ ---
if mod == "eoq":
    st.header("📊 Economic Order Quantity (Wilson Framework)")
    st.write("Balance procurement transaction costs against asset warehousing holding premiums.")
...
# --- MODULE 2: SAFETY STOCK & ROP ---
    ...
    st.header("🛡️ Safety Stock & Reorder Point Engine")
    st.write("Calculate strategic buffer levels to hedge against stochastic demand during supplier lead periods.")
...
# --- MODULE 3: TOTAL COST ---
    ...
    st.header("💰 Total System Inventory Cost Matrix")
...
# --- MODULE 4: CALCULATE ALL MASTER ---
    ...
    st.header("🚀 Calculate All")
    st.write("Enter all your core thresholds below to evaluate every single inventory metric at once.")
    ...
    if calculate_all:
        if H_all == 0:
            ...
            st.markdown("### Primary Optimization Metrics")
            cols_metrics([
                ("Economic Order Qty", f"{eoq_all:,.0f}", "units/order"),
                ("Safety Stock (SS)",  f"{ss_all:.1f}",   "units Buffer"),
                ("Reorder Point (ROP)",f"{rop_all:.1f}",  "inventory scale"),
            ...
            st.markdown("### Operations Framework Breakdown")
            cols_metrics([
                ("Annual Demand Volume", f"{D_all:,.0f}",       "units/year"),
                ("Orders Placed",        f"{orders_all:.1f}",   "times/year"),
                ("Lead Time Demand",     f"{demand_during_lt_all:.1f}", "units"),
            ...
            st.markdown("### System Optimization Visualizations")
            c1, c2 = st.columns(2)
            with c1:
                q_range = np.linspace(eoq_all * 0.2, eoq_all * 3, 200)
                hc_line = (q_range / 2) * H_all
                ...
                fig_all.update_layout(
                    title={"text":"Total Optimization Cost Curves","font":{"size":15}},
                    paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
                    yaxis={"title":"Cost (₦)","gridcolor":"#e2e8f0"},
                    xaxis={"title":"Order quantity Thresholds"},
                    legend={"bgcolor":"rgba(255,255,255,0.8)"},
                    height=320, margin=dict(t=50,b=20,l=20,r=20),
                )
            ...
            with c2:
                ...
                fig_wf.update_layout(
                    title={"text": "Reorder Point (ROP) Structure Balance", "font": {"size": 16}},
                    paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
        ...
        st.info("👈 Complete the global parameter values and click **Run Master Calculation** to get an instant snapshot of your entire optimization portfolio.").hero-title {
    font-size: 38px;
    font-weight: 800;
    color: #1e40af;
    margin: 0;
