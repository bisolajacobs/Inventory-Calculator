...
def cols_metrics(metric_list, ncols=4):
    ...
    for idx, (label, val, unit) in enumerate(metric_list):
        with cols[idx % ncols]:
            st.metric(label=label, value=val, delta=unit, delta_color="off")

def formula_html(steps):
    html = '<table border="1" style="width:100%; border-collapse:collapse; background:white;"><tr style="background-color:#f1f5f9;"><th>Target Identifier</th><th>Calculated Formula Operation Breakdowns</th></tr>'
    for title, formula in steps:
        html += f'<tr><td style="padding:10px;"><b>{title}</b></td><td style="padding:10px;"><code>{formula}</code></td></tr>'
    html += '</table>'
    return html

# ─────────────────────────────────────────────────────────────────────────────
# ── 3. APP HERO BANNER ───────────────────────────────────────────────────────
st.title("📦 Mathematical Inventory Optimization Suite")
st.caption("Enterprise Multi-Model Supply Chain Parameter Calibrator Engine")
st.divider()

# ─────────────────────────────────────────────────────────────────────────────
# ── 4. CONTROL ENGINE SIDEBAR ────────────────────────────────────────────────
modules = [
